...
public class 블록이동하기 {
    ...
    public ArrayList<Node> getNextPos(Node pos, int[][] board){
        ...
        if(pos.getPos1Y() == pos.getPos2Y()){
            for(int i=0;i<2;i++){
                //위쪽 혹은 아래쪽ㄱ 두 칸이 모두 비어 있다면
                if(board[pos.getPos1X()][pos.getPos1Y() + ver[i]] == 0 && board[pos.getPos2X() + hor[i]][pos.getPos2Y()] == 0){
