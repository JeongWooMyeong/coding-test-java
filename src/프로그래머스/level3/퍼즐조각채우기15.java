...
public class 퍼즐조각채우기15 {
    ...
    static boolean match(List<int[]> q, List<int[]> w){
        if(q.size() != w.size()) return false;
        q.sort((a,b)->a[0]==b[0] ? a[1] - b[1] : a[0] - b[0]);
        w.sort((a,b)->a[0]==b[0] ? a[1] - b[1] : a[0] - b[0]);

        for(int i=0;i<q.size();i++){
            if(q.get(i)[0] != w.get(i)[0] || q.get(i)[1] != w.get(i)[1]) return false;
        }
