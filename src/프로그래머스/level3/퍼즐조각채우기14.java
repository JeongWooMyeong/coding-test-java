...
public class 퍼즐조각채우기14 {
    ...
    static boolean match(List<int[]> z, List<int[]> x){
        if(z.size() != x.size()) return false;
        z.sort((a,b)->a[0] == b[0] ? a[1] - b[1] : a[0] - b[0]);
        x.sort((a,b)->a[0] == b[0] ? a[1] - b[1] : a[0] - b[0]);

        for(int i=0;i<z.size();i++){
           if(z.get(i)[0] != x.get(i)[0] || z.get(i)[1] != x.get(i)[1]) return false;
        }
