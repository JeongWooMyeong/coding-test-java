...
public class 기둥과보설치6 {
    ...
    //설치 or 제거 했을때 result에 있는게 기준이 충족하는지 확인
    static boolean can(){
        ...
        for(int[] r : result){
            ...
            if(type == 0){
                ...
                if(y == 0 || contains(x-1,y,1) || contains(x,y,1) || contains(x,y-1,0)) continue;
    ...
    static boolean contains(int x, int y, int type){
        for(int[] r : result){
            if(r[0] == x && r[1] == y && type == r[2]) return true;
        }
        return false;
    }
