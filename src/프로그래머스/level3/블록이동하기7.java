...
public class 블록이동하기7 {
    ...
    static int bfs(int ax, int ay, int bx, int by, int[][] board){
        ...
        while(!q.isEmpty()){
            ...
            for(int i=0;i<4;i++){
                ...
                if(nx1 < 0 || nx2 < 0 || ny1 < 0 || ny2 < 0 || nx1 >= n || ny1 >= m || nx2 >= n || ny2 >= m) continue;
            ...
            if(x1 == x2){
                //위방향
                if(x1-1 >= 0 && board[x1-1][y1] != 1 && board[x1-1][y2] != 1){
                    addState(x1,y1,x1-1,y1,count, q);
                    addState(x2,y2,x2-1,y2,count, q);
                }

                //아래방향
                if(x1+1 < n && board[x1+1][y2] != 1 && board[x1+1][y1] != 1){
                    addState(x1,y1,x1+1,y1,count,q);
                    addState(x2,y2,x2+1,y2,count,q);
                }
            }else{
                //왼쪽
                if(y1-1>=0 && board[x1][y1-1] != 1 && board[x2][y1-1] != 1){
                    addState(x1,y1,x1,y1-1,count,q);
                    addState(x2,y2,x2,y2-1,count,q);
                }

                //오른쪽
                if(y1+1<m && board[x1][y1+1] != 1 && board[x2][y1+1] != 1){
                    addState(x1,y1,x1,y1+1,count,q);
                    addState(x2,y2,x2,y2+1,count,q);
                }
            }
